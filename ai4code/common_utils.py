...
def load_config_data(experiment_name: str, experiments_dir='experiments') -> dict:
    with open(f"{experiments_dir}/{experiment_name}.yaml", 'r') as f:
...
class CosineAnnealingWarmRestarts(torch.optim.lr_scheduler._LRScheduler):
    ...
    def __init__(self, optimizer, T_0, T_mult=1.0, eta_min=0, last_epoch=-1, verbose=False, first_epoch_lr_scale=None, first_steps_scale=None, min_first_steps_scale_epoch=1):
        ...
        self.first_steps_scale = first_steps_scale
        self.min_first_steps_scale_epoch = min_first_steps_scale_epoch
    ...
    def get_lr(self):
        ...
        if self.last_epoch >= self.min_first_steps_scale_epoch and self.first_steps_scale is not None:
            lr_scale *= self.first_steps_scale.get(self.T_cur, 1.0)
...
def check_CosineAnnealingWarmRestarts():
    ...
    scheduler = CosineAnnealingWarmRestarts(
        optimizer, T_0=16, T_mult=1.41421, first_epoch_lr_scale=1./100,
    #    first_steps_scale={0: 1./30, 1: 1./10, 2: 1./3, 3: 1./2}
    )
